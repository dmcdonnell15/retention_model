...
--	, case when pell_recipient_status = 'Pell Recipient' then 1 else 0 end [pell recipient status]
	, case when (bridg.[student id] is not null 
        ...
        and t1.[home college] = 'TR' then 1 else 0 end 
...
/*GPA at term start (GPA institutional cum gets recalculated when term ends)*/
left join
(select [student id], [gpa institutional cumulative], term
, LAG([gpa institutional cumulative], 1) OVER (partition by [student id] ORDER BY [term order]) [GPA at start of term]
from openbook.dbo.pvt_studentterms
where [Enrolled] = 'yes' and [instructional area] = 'semester credit') gpa
on t1.[student id] = gpa.[student id] and t1.Term = gpa.term
...
group by t1.[student id], t2.[Student ID], [GPA at start of term], [start of term retention status], tests.[Student ID], waiver.[Student ID], t1.Term, t1.[Home College]
	, [First_reg], t1.[Gender], [age at census], t1.Ethnicity, t1.[STAR Eligibility], [Declared Degree], [Full or Part Time], [Ever Early College]
	, t1.[Athletic Indicator], pelig.[Student ID], [Starting Cohort Term], gwy.[Student ID], bridg.[Student ID], gwy.[Student ID]
